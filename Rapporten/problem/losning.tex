...
Dog vil der først findes en vektor AP mellem punkterne A og P med formlen:  %(AP) ⃗=■(X2-X1@Y2-Y1)
Vektor AP: %A(1, 5) og P(5, 4): (AP) ⃗=■(5-1@4-5)= ■(4@-1)
Vektor AB findes også da det næste trin er at projektere AP på AB. %A(1, 5) og B(7, 5): (AP) ⃗=■(7-1@5-5)= ■(6@0)
For at projektere AP på AB skal følgende formel benyttes: %ba = ((a * b) / |a|2) * a.
Med denne formel vil vektoren b blive projekteret på vektoren a. I tælleren findes prikproduktet som kan findes ved at: % a∙b=■(X1∙X2@Y1∙Y2)
I nævneren findes længden på vektor a i anden, som kan regnes ved at sige: % 〖√(〖ax〗^2+〖ay〗^2 )〗^2
Hvis der forsat kigges på eksemplet med figur X, vil projektionen af AP på AB se således ud:
Prikproduktet af vektorerne AP og AB: % ■(   4&6@-1&0) = (4∙6)+(-1∙0)=24+0=24
Længden af AB vil være: % 〖√(6^2+0^2 )〗^2= 〖√(36+0)〗^2=36
Ud fra dette kan vektoren fra projektionen af AP på AB findes: % 24/36∙■(6@0)  24/36∙6=144/36=4 og 24/36∙0=0/36=0
...
Hvor resultatet vil give en ny vektor %■(4@0) som også vil have startpunkt i A. Hvis der igen kigges på formlen
%0 < AP ∙ AB < AB ∙ AB ᴧ 0 < AP ∙ AD < AD ∙ AD
overholder punktet P første del, og ovenstående metode skal derfor gentages med vektoren AD i stedet for AB, for matematisk at finde ud af om punktet ligger inden 	for den afsatte bredde og længden af ruten a til b. Ved udregning af projektionen af AP på AD vil den nye vektor hedde	% ■(   0@-1).
